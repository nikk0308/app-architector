...
collect_remote_diagnostics() {
  ...
  journalctl -u "$SERVICE_NAME" --no-pager -n 250 > "$DIAG_DIR/journal.txt" 2>&1 || true
  nginx -t > "$DIAG_DIR/nginx-test.txt" 2>&1 || true
  ss -ltnp > "$DIAG_DIR/ports.txt" 2>&1 || true
  curl -v --max-time 5 "http://127.0.0.1:$API_PORT/api/health" > "$DIAG_DIR/remote-health.txt" 2>&1 || true
  find "$APP_ROOT" -maxdepth 4 -type f -printf '%p %s bytes\n' > "$DIAG_DIR/remote-app-tree.txt" 2>&1 || true
  find "$RELEASE_DIR" -maxdepth 4 -type f -printf '%p %s bytes\n' > "$DIAG_DIR/release-tree.txt" 2>&1 || true
  [ -f "$RELEASE_DIR/apps/api/dist/index.js" ] && head -n 40 "$RELEASE_DIR/apps/api/dist/index.js" > "$DIAG_DIR/api-dist-head.txt" 2>&1 || true
  set -e
}
...
# Runtime dependencies are installed on the target server only after the compiled release is uploaded.
# The log is persisted so a failed native dependency install is visible in the GitHub artifact.
...
if [ -n "${RUNTIME_ENV_B64:-}" ]; then
  printf '%s' "$RUNTIME_ENV_B64" | base64 -d >> "$SHARED_DIR/.env"
  printf '\n' >> "$SHARED_DIR/.env"
fi
...
sed \
  ...
  -e "s|__API_PORT__|$API_PORT|g" \
  "$NGINX_TEMPLATE" > "/etc/nginx/sites-available/$DOMAIN"
...
# Publish frontend only after API is alive. This prevents the visible site from updating while the backend is broken.
...
# Keep the last 5 releases to avoid unbounded disk growth.
