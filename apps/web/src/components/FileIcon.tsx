...
function badgeFor(path: string): string {
  ...
  if (ext === "cs") return "C#";
  if (ext === "h") return "H";
  if (["m", "mm"].includes(ext)) return "OC";
  ...
  if (["env", "config", "conf"].includes(ext)) return "CFG";
  if (ext === "prefab") return "PRF";
  if (ext === "unity") return "SCN";
  if (ext === "asmdef") return "ASM";
  if (ext === "controller") return "CTR";
  if (ext === "anim") return "ANM";
  if (["asset", "mat", "fbx", "meta"].includes(ext)) return "AST";
  if (["shader", "hlsl", "cginc"].includes(ext)) return "SHD";
  if (["png", "jpg", "jpeg", "webp", "svg"].includes(ext)) return "IMG";
  if (["wav", "mp3", "ogg"].includes(ext)) return "AUD";
  if (["mp4", "mov"].includes(ext)) return "VID";
  if (["zip", "ipa", "aab", "apk"].includes(ext)) return "PKG";
  if (ext === "txt") return "TXT";
  if (ext === "csv") return "CSV";
  if (ext === "sql") return "SQL";
  if (name === "package.json") return "PKG";
  if (name === "pubspec.yaml") return "PUB";
  if (name === "project.yml") return "PRJ";
  return "FILE";
}
